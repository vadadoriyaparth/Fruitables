...
function Product() {
    ...
    const productSchema = object({
        name: string().required("Enter youre name "),
        description: string().required("Enter youre description "),
        price: number().required("Enter youre price "),
        category_id: string().required("Enter youre category"),
        subcategory_id: string().required("Enter youre  subcategory"),
        image: string().required("Upaloade youre subcategory")
        .test("fileType","the file is large",(values)=>{
            console.log(values);
            if (values?.size) {
                return values.size <= 2 * 1024 * 1024;
            }
            return true;
        })
        .test("fileType","The file type is not supported",(values)=>{
            if (values?.type) {
                return["image/jpeg","image/png","image/jpg"].includes(values.type);
            }
            return true;
        })
    });
